...
M.augroup = function(args)
  ...
  vim.validate({
    group = {
      group,
      function(s)
        return type(s) == "string" and not group_forbidden_pattern:match_str(s)
      end,
      "valid augroup name",
    },
  })
  ...
  local register_cmd = function(event, pattern, cmd)
    vim.validate({
      event = {
        event,
        function(a)
          return type(a) == "string" and string.match(a, "%s") == nil
        end,
        "valid event name",
      },
      ...
      cmd = {
        cmd,
        function(a)
          return type(a) == "string" or type(a) == "function"
        end,
    ...
    table.insert(
      definition,
      string.format(
        [[  au %s %s lua _autocmd_lua._execute_group('%s', %d)]],
        event,
        pattern,
        group,
        num
...
M._execute_group = function(group, num)
  ...
  if cmd == nil then
    error(
      string.format(
        "autocmd-lua: Unknown command in group %s (number %d)",
        group,
        num
      )
    )
  ...
  if type(cmd) == "function" then
    ...
    if not ok then
      error(
        string.format(
          "autocmd-lua: Error in autocmd group %s, msg: %s",
          group,
          error_msg
        )
      )
